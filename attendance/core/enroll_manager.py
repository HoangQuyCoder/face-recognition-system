...
class EnrollManager:
    # ================= CONFIG =================
    MIN_CONFIDENCE = 0.60          # ← Giảm từ 0.65 để dễ hơn
    MIN_SAMPLE_SIMILARITY = 0.70   # ← Giảm từ 0.75 để dễ thu thập hơn
    MAX_OUTLIER_DISTANCE = 0.40    # ← Tăng từ 0.35 để ít loại bỏ hơn
    # =========================================
    ...
    def add_frame(self, rgb_frame):
        ...
        if face.det_score < self.MIN_CONFIDENCE:
            print(f"⚠ Low confidence: {face.det_score:.3f} < {self.MIN_CONFIDENCE}")
            return False
        ...
        if self.last_embedding is not None:
            ...
            if sim > self.MIN_SAMPLE_SIMILARITY:
                print(f"⚠ Too similar to last sample: {sim:.3f} > {self.MIN_SAMPLE_SIMILARITY}")
                return False
        ...
        print(f"✅ Sample #{len(self.samples)} added! ({len(self.samples)}/{self.max_samples})")
        return True
    ...
    def is_complete(self):
        ...
        if complete:
            print(f"🎉 Enrollment complete! {len(self.samples)}/{self.max_samples} samples collected")
        return complete
    ...
    def _remove_outliers(self, embeddings):
        ...
        if len(embeddings) < 5:
            print(f"ℹ Too few samples ({len(embeddings)}) to remove outliers, keeping all")
            return embeddings
        ...
        if len(filtered) < 5:
            print(f"⚠ Too many outliers ({len(removed_indices)}), keeping all samples")
            return embeddings
        
        print(f"✓ Outlier removal: kept {len(filtered)}/{len(embeddings)} samples")
        if removed_indices:
